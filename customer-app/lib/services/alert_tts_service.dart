...
/// Centralized Cross-Platform Alert-Only Text-to-Speech (TTS) Service for Sahkar Sewa.
/// 
/// Strictly for critical alerts/events (e.g. booking accepted, worker arrival, payment, SOS).
/// Does NOT perform screen narration, element reading, or voice navigation.
/// 
/// Supported Platforms:
/// - Chrome & Microsoft Edge (Web Speech API)
/// - Android (Native Android TTS)
class AlertTtsService {
  static const String _prefAlertVoice = 'alert_voice_enabled';
  ...
  // In-memory sliding window for alert deduplication (keeps spoken alert IDs for 60 seconds)
  ...
  static Future<void> initialize() async {
    ...
    try {
      ...
      // Configure default speech rate and volume suitable for alerts
      await _flutterTts!.setSpeechRate(kIsWeb ? 0.95 : 0.5);
      await _flutterTts!.setVolume(1.0);
      await _flutterTts!.setPitch(1.0);

      // Handle completion/error safely
      _flutterTts!.setErrorHandler((msg) {
        if (kDebugMode) {
          debugPrint('[AlertTtsService] Engine error: $msg');
        }
      });

      _isInitialized = true;
      if (kDebugMode) {
        debugPrint('[AlertTtsService] Initialized successfully. Voice enabled: $_isEnabled');
      }
    } catch (e) {
      if (kDebugMode) {
        debugPrint('[AlertTtsService] Initialization failed gracefully: $e');
      }
      _isInitialized = false;
    }
  }

  /// Whether Alert Voice is enabled by the user
  ...
  static Future<void> setEnabled(bool enabled) async {
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('[AlertTtsService] Failed to save alert voice setting: $e');
      }
    }
  }
  ...
  static Future<void> speakAlert({
    ...
    if (_spokenAlertHistory.containsKey(alertId)) {
      if (kDebugMode) {
        debugPrint('[AlertTtsService] Dropping duplicate alert speech: $alertId');
      }
      return;
    }
    ...
    try {
      ...
      // If exact query failed, attempt setting primary without crashing
      ...
      if (kDebugMode) {
        debugPrint('[AlertTtsService] Spoke alert [$alertId]: "$localizedText"');
      }
    } catch (e) {
      // Speech failure must never crash or block the application
      if (kDebugMode) {
        debugPrint('[AlertTtsService] Speech failed gracefully: $e');
      }
    }
  }
  ...
  /// Optional warm-up call triggered by legitimate user gesture
